...
describe('find-iconset-folders', () => {
  ...
  it('should be able to find the React Native iconset', async () => {
    ...
    expect(iconsets).to.include(path.normalize('test/ReactNativeIconTest/ios/ReactNativeIconTest/Images.xcassets/IconMagick.appiconset'));
  });
  ...
  it('should be able to find the Cordova iconset', async () => {
    ...
    expect(iconsets.length).to.equal(1);
    expect(iconsets).to.include(path.normalize('test/CordovaApp/platforms/ios/ionic_app/Images.xcassets/IconMagick.appiconset'));
  });

  it('should be able to find the Native iconset', async () => {
    const iconsets = await findIconsetFolders('./test/NativeApp');
    expect(iconsets.length).to.equal(1);
    expect(iconsets).to.include(path.normalize('test/NativeApp/ios/native_app/Assets.xcassets/IconMagick.appiconset'));
  });
});
